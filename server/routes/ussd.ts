...
export const handleUssd: RequestHandler = (req, res) => {
  const { text = "", phoneNumber = "" } = (req.method === "GET" ? req.query : req.body) as any;
  ...
  if (input.startsWith("1")) {
    return res.type("text/plain").send(
      "END Advice: Irrigate 10mm this week. Pest risk: Low",
    );
  }
  if (input.startsWith("2")) {
    return res.type("text/plain").send(
      "END Buyers near you: Mandi Co-Op (₹18–₹22/kg)",
    );
  }
  if (input.startsWith("3")) {
    return res.type("text/plain").send(
      "END Send SMS 'RECORD WHEAT 1-ACRE' to provided number",
    );
  }
