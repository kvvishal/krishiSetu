...
export const handleAdvice: RequestHandler = (req, res) => {
  ...
  let irrigationMmWeek = Math.max(
    0,
    Math.round((25 - rainfall3d * 0.8) * soilFactor),
  );
  if (moisture > 45) irrigationMmWeek = 0;

  const pestRisk =
    moisture > 50 || rainfall3d > 40
      ? "High"
      : rainfall3d > 20
        ? "Medium"
        : "Low";
  ...
  const advice = {
    ...
    tips: [
      irrigationMmWeek === 0
        ? "Skip irrigation this week"
        : `Irrigate ${irrigationMmWeek}mm this week`,
      pestRisk === "High"
        ? "Scout daily for aphids and rust; use traps"
        : "Monitor weekly for pests",
      soil === "Sandy"
        ? "Add organic matter to improve water holding"
        : soil === "Clay"
